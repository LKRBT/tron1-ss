...
static const std::string ACCID = "WF_TRON1A_282";

// TODO : 실패 또는 완료시 푸쉬
static const std::unordered_set<std::string> NOTIFY_WITH_RESULT = {
    "notify_stand_mode", // 실패 또는 완료시 푸쉬
    "notify_walk_mode",  // 실패 또는 완료시 푸쉬
    "notify_twist",      // 실패시 푸쉬
    "notify_sitdown",    // 실패시 푸쉬
    "notify_recover",
    // 일단, 별도 처리 안하는 항목은 로깅
    "notify_odom",
    "notify_imu"
...
static const std::unordered_set<std::string> RESPONSE_WITH_RESULT = {
    "response_stand_mode",
    "response_walk_mode",
    "response_base_height",
    "response_sitdown",
    "response_stair_mode",
    // TODO : mark time
    "response_emgy_stop",
    "response_enable_odom",
    "response_enable_imu",
    "response_recover"
    // TODO : light effect
};
...
static void handle_commands() {
    while (!should_exit) {
        std::string command;
        std::cout << "Enter command ('stand', 'walk', 'twist', 'base_height', 'sit', 'stair', 'stop', 'odom', 'imu', 'recover') or 'exit' to quit:" << std::endl;
        ...
        else if (command == "base_height") {
            int direction;
            std::cout << "Increase height 1, Decrease height -1" << std::endl;
            std::cin >> direction; // Get direction flag from user
            send_request("request_base_height", {{"direction", direction}});
        }
        else if (command == "sit") {
        ...
        // TODO : mark time
            ...
            bool enable;
            std::cout << "Enable odom (true/false):" << std::endl;
            std::cin >> enable;  // Get odom enable flag from user
            send_request("request_enable_odom", {{"enable", enable}});
        }
        else if (command == "imu") {
            std::string enable;
...
// TODO : handle_notify_odom

static void handle_notify_message(const std::string& title, const json& msg) {
    if (NOTIFY_WITH_RESULT.count(title)) {
        std::string result = "unknown";
        if (msg.contains("data") && msg.at("data").contains("result")) {
            result = msg.at("data").at("result").get<std::string>();
        }
        std::cout << "title = " << title << ", result = " << result << std::endl;
    }
    // TODO
    else if (title == "notify_robot_info") {
        // handle_notify_robot_info(msg)
        json data = msg.contains("data") ? msg.at("data") : json();
        std::cout << "[ROBOT INFO] data = " << data.dump() << std::endl;
    }
    // TODO
    // else if (title == "notify_odom") {
    //     handle_notify_odom(msg);
    // }
    // TODO
    // else if (title == "notify_imu") {
    //     handle_notify_imu(msg)
    // }
    else if (title == "notify_invalid_request") {
        json data = msg.contains("data") ? msg.at("data") : json();
        std::cout << "[INVALID REQUEST] data = " << data.dump() << std::endl;
    }
    else {
        std::cout << "[NOT DEFINED] title=" << title << std::endl;
    }
}

static void handle_response_message(const std::string& title, const json& msg) {
    if (RESPONSE_WITH_RESULT.count(title)) {
        std::string result = "unknown";
        if (msg.contains("data") && msg.at("data").contains("result")) {
            result = msg.at("data").at("result").get<std::string>();
        }
...
static void on_message(connection_hdl hdl, client<websocketpp::config::asio>::message_ptr message) {
    // std::cout << "Received: " << message->get_payload() << std::endl;  // Print received message
    // try {
    //     // Parse JSON data from message payload
    //     json msg = json::parse(message->get_payload());

    //     std::string title = msg.contains("title") ? msg.at("title").get<std::string>() : "";
        
    //     if (title.rfind("notify_", 0) == 0) {
    //         handle_notify_message(title, msg);
    //     }
    //     else if (title.rfind("response_", 0) == 0) {
    //         handle_response_message(title, msg);
    //     }
    //     else {
    //         std::cout << "[UNHANDLED] title=" << title << std::endl;
    //     }
    // }
    // catch (const std::exception& e) {
    //     std::cout << "[Error] on_message parse failed: " << e.what() << std::endl;
    // }
}
